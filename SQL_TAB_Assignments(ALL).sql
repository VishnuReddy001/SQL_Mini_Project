...
group by Month having Total_Payments > 20 order by Total_Payments desc;

drop database if exists customers_orders;
CREATE DATABASE IF NOT EXISTS Customers_Orders;
use Customers_Orders;
Drop table if exists customers;
create table if not exists customers (customer_id int primary key auto_increment,
...
insert into project values (1, "Pranaya", "Male", 3),
                           ...
                           (7, "Hina", "Female", 3);
select * from project;
select a.fullName as ManagerName, b.fullName as EmployeeName from project a
...
SELECT pl.productLine AS category_name,
...
CREATE PROCEDURE Get_country_payments(
    IN YearInput INT,
    IN CountryInput VARCHAR(100)
)
BEGIN
    -- Select total payments for a specific year and country
    SELECT 
...
END $$
DELIMITER ;
SHOW PROCEDURE STATUS WHERE Db = 'classicmodels';  -- Replace 'classicmodels' with your database name
...
MonthlyOrdersWithYoY AS (
    SELECT
        mo.Year,
        mo.Month,
        mo.TotalOrders,
        -- Using LAG to get the previous year's orders for the same month
        LAG(mo.TotalOrders) OVER (ORDER BY mo.Year, MONTH(STR_TO_DATE(mo.Month, '%M'))) AS PreviousYearOrders,
        -- Calculating YoY Percentage Change, but keeping NULL for first month (January 2003)
        CASE 
            WHEN mo.Year = 2003 AND mo.Month = 'January' THEN NULL
            ELSE CONCAT(FORMAT(
                ((mo.TotalOrders - LAG(mo.TotalOrders) OVER (ORDER BY mo.Year, MONTH(STR_TO_DATE(mo.Month, '%M')))) 
                / LAG(mo.TotalOrders) OVER (ORDER BY mo.Year, MONTH(STR_TO_DATE(mo.Month, '%M')))) * 100, 
                0), '%')
        END AS YoYChange
    FROM
        MonthlyOrders mo
...
SELECT
    ...
    YoYChange AS "% YoY Change"
FROM
    MonthlyOrdersWithYoY
ORDER BY
    Year,
...
DROP PROCEDURE IF EXISTS InsertIntoEmp_EH;
DELIMITER $$
...
CALL InsertIntoEmp_EH(1, 'Jane Smith', 'jane.smith@example.com');
...
Create table Emp_BIT ( Name varchar(50),
                       ...
                       Working_hours int);
DELIMITER //
CREATE TRIGGER before_emp_bit_insert
